<template>
  <div id="mypage">
    ...
    <ul class="wrap_list">
      <li class="list" v-for="(item,index) in this.$store.state.notifications" :key="index"><a
        :href="item.subject.url.replace('api.','').replace('/repos','')"><i>{{item.subject.type}}</i>{{item.subject.title}}</a>
      </li>
    </ul>
    <el-button @click="logout">ログアウト</el-button>
    <notifications position="top right" group="notifications"/>
  </div>
</template>
